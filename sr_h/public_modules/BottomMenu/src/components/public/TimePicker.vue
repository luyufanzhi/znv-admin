...
export default {
	props: ['defaultNo', 'title', "area", "word"],
	created() {
		this.areaArr = this.area;
		if (this.defaultNo < 60) {
			this.stepone = 5;
		} else if (this.defaultNo < 120 && this.defaultNo > 60) {
			this.stepone = 10;
		} else {
			this.stepone = 30;
		}

		for (var i = Number(this.areaArr[0]); i <= this.areaArr[1];) {
			if (i < 60) {
				this.stepone = 5;
			} else if (i < 120 && i >= 60) {
				this.stepone = 10;
	...
	methods: {
		...
		setDefaultTime() {
			//设置picker中默认选中的状态
			if (this.defaultNo) {
				// var defaultIndexone = (this.defaultNo-this.areaArr[0])/this.stepone;
				// var defaultIndextwo = (this.defaultNo[1]-this.areaArr[2])/this.steptwo;
				let num = this.slots[0].values.indexOf(this.defaultNo[0])
				this.slots[0].defaultIndex = num;
		...
		changeColor() {
			...
			var activeIndexmin = this.slots[0].values.indexOf(this.valueList)
			if ( activeIndexmin == -1 ) {
				return;
			}
			this.itemmin[activeIndexmin].innerHTML = this.itemmin[activeIndexmin].innerHTML;
...
.selectone .picker-item.picker-selected::after {
	position: relative;
	left: 1px;
	content: "秒"
}
