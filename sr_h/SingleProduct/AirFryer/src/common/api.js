...
// const KEY       = 'hilink';
const METHODS = 'getDevCacheAll setDeviceInfo getDevInfo modifyTitleStatus modifyTitleBar ' +
    'addMessage removeMessage jumpTo getDevHistory setTitleVisible setTitleEnable';
...
export default function ( services, name ) {
    ...
    // 使用到的临时变量
    let sid, data, key, sendStr;

    // 缓存, 用来修改下发
    let cacheData = extend( [ ], services ), oldData = extend( [ ], services );
    ...
    extend( api, {
        ...
        sid: { }, // 所有 服务 id 集合
        oldData, // 缓存旧值, 下发失败时还原

        /* 获取全局 */
        getDevCacheAll () {
            console.warn( 'getDevCacheAll' );
            catchFn(
                () => hilink.getDevCacheAll( '0', '', filterName( 'getDevCacheAll' ) ),
                'getDevCacheAll',
                null
            );
        },
        ...
        setDeviceInfo ( data, keys ) {
            ...
            catchFn(
                () => hilink.setDeviceInfo( '0', sendStr, filterName( 'setDeviceInfo' ) ),
                'setDeviceInfo',
                data
            );
        },
        ...
        getDevInfo ( key = '', sid = services[ 0 ].sid ) {
            catchFn(
                () => hilink.getDevInfo( '0', sid, `{'${key}'}`, filterName( 'getDevInfo' ) ),
                'getDevInfo'
            );
        },
        ...
        modifyTitleStatus ( data ) {
            ...
            catchFn(
                () => hilink.modifyTitleStatus( data, filterName( 'modifyTitleStatus' ) ),
                'modifyTitleStatus'
            );
        },
        ...
        modifyTitleBar ( color ) {
            ...
            catchFn(
                () => hilink.modifyTitleBar( true, color, filterName( 'modifyTitleBar' ) ),
                'modifyTitleBar'
            );
        },
        ...
        removeMessage ( data ) {
            ...
            catchFn(
                () => hilink.removeMessage( stringify( data ), filterName( 'removeMessage' ) ),
                'removeMessage'
            );
        },
        ...
        addMessage ( data ) {
            ...
            catchFn(
                () => hilink.addMessage( stringify( data ), filterName( 'addMessage' ) ),
                'addMessage'
            );
        },
        ...
        jumpTo ( url ) {
            ...
            catchFn(
                () => hilink.jumpTo( url, filterName( 'jumpTo' ) ),
                'jumpTo'
            );
        },

        /* 设置 title 是否可见 */
        // true，标题可见，false标题不可见
        setTitleVisible ( flag ) {
            flag = !!flag;
            console.warn( 'setTitleVisible:', flag );
            catchFn(
                () => hilink.setTitleVisible( flag, filterName( 'setTitleVisible' ) ),
                'setTitleVisible'
            );
        },
        ...
        setTitleEnable ( flag ) {
            flag = !!flag;
            console.warn( 'setTitleEnable:', flag );
            catchFn(
                () => hilink.setTitleEnable( flag, filterName( 'setTitleEnable' ) ),
                'setTitleEnable'
            );
        },
		...
		*/
        ...
        getDevHistory ( {
            ...
            character = '' } ) {
            catchFn(
                () => hilink.getDevHistory(
                    0, pageNo, pageSize, startTime, endTime, sid, character,
                    name + '.getDevHistory' + CALLBACK ),
                'getDevHistory'
            );
        },
    ...
    METHODS.split( SPACE_RGE ).forEach( el => {
        ...
        api.callbacks[ key ] = res => {
            ...
            if ( 0 === code ) { 		// 成功
                ...
                if ( type( api.success ) === 'function' ) {
                    api.success( services );
                }
                return;
            } else {					// 失败
                console.warn( '下发失败, errorCode: ', res.errcode );
                if ( type( api.error ) === 'function' ) {
                    api.error( services );
                }
                api.commitAll( oldData );
    ...
    function setIsReset () {
        window.setTimeout( () => api.isReset = true, 300 );
    }
