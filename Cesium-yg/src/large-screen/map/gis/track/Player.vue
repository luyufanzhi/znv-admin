<template>
    <div class="gis-player unselect">
        ...
        <div class="progress">
            <el-slider v-model="curTime" @change="curTimeChange"></el-slider>
...
export default {
    ...
    created () {
        ...
        this.$axios.get( '/static/mork/playback.json' ).then( ( data ) => {
    ...
    methods: {
        ...
        register () {
            ...
            this.track.on( 'update', ( { point, time } ) => {
                this.curTime = (
                    ( time - this.track.startTime.getTime() ) /
                    this.track.totalTime
                ) * 100
            } )
        },
...
.gis-player {
    width: .px2vw(573)[@result];
    height: .px2vh(130)[@result];
    background:rgba(7,32,69,1);
    border:1px solid rgba(47,184,253,1);
    .shadow;
    border-radius:5px;
    padding: .px2vh(11)[@result] .px2vw(11)[@result];

    .title {
        font-size: .px2vh(16px)[@result];
        color: #fff;
    }

    .progress {
        width: 100%;
        padding: 0 .px2vh(11)[@result];
        .setRelative();
        margin-bottom: .px2vh(20)[@result];

        @import "~@less/element/progress.less";

        .time {
            position: absolute;
            bottom: .px2vh(-5)[@result];
            transform: translateY(-100);
            color: #fff;
            font-size: .px2vh(12)[@result];
        }
        .time.start {
            left: 0;
        }
    ...
    .options {
        ...
        .item {
            margin-right: .px2vw(10)[@result];
        }

        .btn {
            width: .px2vw(65)[@result];
